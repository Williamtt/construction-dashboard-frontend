...
import { Loader2, Eye, ScrollText, Search } from 'lucide-vue-next'
  ...
  <div class="space-y-4">
    <div>
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        檢視平台關鍵操作紀錄（租戶、使用者、專案等），可依關鍵字、動作、資源類型、時間區間篩選。
      </p>
    </div>
    ...
    <div class="rounded-lg border border-border bg-card p-4">
      ...
      <template v-else>
        ...
        >
          <ScrollText class="size-10 opacity-50" />
          <p class="text-sm">尚無稽核紀錄或目前篩選無結果。</p>
        </div>
      </template>
    </div>
          ...
          <template v-if="hasBeforeAfter(selectedDetailRow.details)">
            <div class="grid gap-4 sm:grid-cols-2">
              <div class="space-y-2">
                ...
                <pre
                  class="max-h-64 overflow-auto rounded-md border border-border bg-muted/30 p-3 text-xs text-foreground whitespace-pre-wrap break-words"
